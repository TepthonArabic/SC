...
App_Users.__table__.create(checkfirst=True)
...
def get_user_id(message_id):
    try:
        if _result := (
            SESSION.query(App_Users)
            .filter(App_Users.message_id == str(message_id))
            .all()
        ):
            return _result
        return None
...
def del_user_from_db(message_id):
    try:
        if _result := (
            ...
            for rst in _result:
                rem = SESSION.query(App_Users).get((str(rst.message_id), rst.result_id))
                SESSION.delete(rem)
                SESSION.commit()
            return True
        return False
...
def get_user_reply(reply_id):
    try:
        if _result := (
            SESSION.query(App_Users).filter(Bot_Users.reply_id == str(reply_id)).all()
        ):
            return _result
        return None
    finally:
...
def get_user_results(result_id):
    try:
        if _result := (
            SESSION.query(App_Users).filter(Bot_Users.result_id == str(result_id)).all()
        ):
            return _result
        return None
    finally:
...
def get_user_logging(logger_id):
    try:
        if _result := (
            SESSION.query(App_Users).filter(Bot_Users.logger_id == str(logger_id)).all()
        ):
            return _result
        return None
    finally:
